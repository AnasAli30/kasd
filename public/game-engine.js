class SpaceJumpGameClass {
    constructor() {
        ...
        this.breakablePlatformChance = 0.1;
    ...
    generatePlatform(index) {
        ...
        if (index > 5) {
            ...
            if (rand < this.breakablePlatformChance) {
                type = 'breakable';
                color = '#ff6464';
                asset = 'tileRed';
            } else if (rand < this.breakablePlatformChance + this.movingPlatformChance) {
                type = 'moving';
                color = '#64ff64';
                asset = 'tileYellow';
            }
        ...
        // Add shoes powerup rarely (3% chance) - only if no coin
        else if (Math.random() < 0.03) {
            this.createShoesPowerup(platform);
        }
    }
    ...
    createShoesPowerup(platform) {
        const shoes = {
            x: platform.x + platform.width / 2 - 20, // Center on platform
            y: platform.y - 40, // Above platform
            width: 40,
            height: 29, // 116/4 = 29 height per frame
            frame: 0,
            animationTimer: 0,
            collected: false,
            platformY: platform.y // Reference for collision
    ...
    updateShoes() {
        this.shoes.forEach(shoes => {
            if (!shoes.collected) {
                // Animate shoes
                shoes.animationTimer++;
                if (shoes.animationTimer > 8) {
                    shoes.frame = (shoes.frame + 1) % 4; // 4 frames
                    shoes.animationTimer = 0;
                }
                
                // Add floating effect
                shoes.y += Math.sin(Date.now() * 0.003 + shoes.x * 0.01) * 0.3;
            }
        });
        
        // Remove collected shoes and shoes that are too far down
        this.shoes = this.shoes.filter(shoes => 
            !shoes.collected && shoes.y < this.camera.y + this.canvas.height + 200
        );
    }
    ...
    checkShoesCollisions() {
        this.shoes.forEach(shoes => {
            if (!shoes.collected) {
                // Check collision with player
                if (this.player.x < shoes.x + shoes.width &&
                    this.player.x + this.player.width > shoes.x &&
                    this.player.y < shoes.y + shoes.height &&
                    this.player.y + this.player.height > shoes.y) {
                    
                    // Collect shoes powerup
                    shoes.collected = true;
                    
                    // Give super jump boost (5-6 tiles high)
                    this.player.velocityY = -25; // Much stronger than normal jump (-15)
                    this.player.onGround = false;
                    
                    // Create special powerup particles
                    this.createPowerupParticles(shoes.x + shoes.width/2, shoes.y + shoes.height/2);
                    
                    // Play jump sound for the super jump
                    this.playJumpSound();
                }
            }
        });
    ...
    updateGameSpeed() {
        // Increase speed based on score - more gradual progression
        const baseSpeed = 1;
        const speedIncrease = Math.floor(this.score / 200) * 0.05; // +0.05 speed every 200 points (10 coins)
        this.speedMultiplier = Math.min(baseSpeed + speedIncrease, 1.4); // Cap at 1.4x speed
        
        // Apply speed more conservatively to maintain natural feel
        this.gravity = 0.8 * this.speedMultiplier;
        this.jumpForce = -15 * this.speedMultiplier;
        
        // Make platforms more challenging instead of just faster
        this.increaseDifficulty();
    }
    
    increaseDifficulty() {
        // Make game tougher through other means than just speed
        const difficultyLevel = Math.floor(this.score / 300); // Every 300 points = new difficulty level
        
        // Adjust platform spacing (make gaps slightly larger)
        this.platformSpacing = 100 + (difficultyLevel * 5); // Start at 100, increase by 5 each level
        
        // Increase chance of moving/breakable platforms
        this.movingPlatformChance = Math.min(0.15 + (difficultyLevel * 0.05), 0.35); // Max 35%
        this.breakablePlatformChance = Math.min(0.1 + (difficultyLevel * 0.03), 0.25); // Max 25%
    }

    getCurrentBackground() {
        // Get background transition progress for smooth blending
        const scoreProgress = this.score / 300; // How far through current background phase
        const backgroundPhase = Math.floor(scoreProgress) % 3;
        const transitionProgress = scoreProgress - Math.floor(scoreProgress); // 0-1 progress within phase
        
        return {
            current: this.getBackgroundName(backgroundPhase),
            next: this.getBackgroundName((backgroundPhase + 1) % 3),
            transitionProgress: transitionProgress
        };
    }
    
    getBackgroundName(phase) {
        switch (phase) {
    ...
    createPowerupParticles(x, y) {
        // Create special blue/white particles for powerup collection
        for (let i = 0; i < 15; i++) {
            this.particles.push({
                x: x + (Math.random() - 0.5) * 40,
                y: y,
                ...
                maxLife: 40,
                alpha: 1,
                color: Math.random() > 0.5 ? 'rgba(0, 255, 255, 1)' : 'rgba(255, 255, 255, 1)', // Cyan/White
    ...
    generateMorePlatforms() {
        ...
        if (playerY < highestPlatform + 2000) {
            ...
            for (let i = 1; i <= 15; i++) {
                ...
                // Same platform type logic as initial generation - with dynamic difficulty
                if (i > 5) {
                    const rand = Math.random();
                    if (rand < this.breakablePlatformChance) {
                        type = 'breakable';
    ...
    renderBackground() {
        // Get background transition info
        const bgInfo = this.getCurrentBackground();
        const currentBg = this.assets[bgInfo.current];
        const nextBg = this.assets[bgInfo.next];
        ...
        if (currentBg && currentBg.complete) {
            ...
            this.ctx.globalAlpha = 1 - bgInfo.transitionProgress;
            ...
            // Draw next background with transition
            if (nextBg && nextBg.complete && bgInfo.transitionProgress > 0.2) {
                const nextBgScale = Math.max(this.canvas.width / nextBg.width, this.canvas.height / nextBg.height);
                const nextScaledWidth = nextBg.width * nextBgScale;
                const nextScaledHeight = nextBg.height * nextBgScale;
                
                // Smooth transition - start at 20% progress, full opacity at 80%
                const transitionAlpha = Math.min((bgInfo.transitionProgress - 0.2) / 0.6, 1);
                this.ctx.globalAlpha = transitionAlpha;
                
                for (let y = -nextScaledHeight; y <= this.canvas.height + nextScaledHeight; y += nextScaledHeight) {
                    this.ctx.drawImage(
                        nextBg,
                        (this.canvas.width - nextScaledWidth) / 2,
                        y + bgY,
                        nextScaledWidth,
                        nextScaledHeight
                    );
    ...
    renderShoes() {
        this.shoes.forEach(shoes => {
            if (!shoes.collected) {
                const screenY = shoes.y - this.camera.y;
                ...
                if (screenY > -50 && screenY < this.canvas.height + 50) {
                    ...
                    if (this.assets.shoes && this.assets.shoes.complete) {
                        const frameWidth = 116; // 464px / 4 frames = 116px per frame
                        const frameHeight = 116; // Full height
                        const sourceX = shoes.frame * frameWidth;
                        
                        // Add special glow effect for powerup
                        ...
                        this.ctx.drawImage(
                            ...
                            frameWidth, frameHeight,
                            shoes.x, screenY,
                            shoes.width, shoes.height
                        );
                    } else {
                        // Fallback: blue/cyan rectangle with glow
                        this.ctx.fillStyle = '#00FFFF';
                        this.ctx.shadowColor = '#00FFFF';
                        this.ctx.shadowBlur = 10;
                        this.ctx.fillRect(shoes.x, screenY, shoes.width, shoes.height);
                    }
...
window.SpaceJumpGameClass = SpaceJumpGameClass;
